...
def test_add_calculation_execute_positive(mock_addition):
    ...
    # Act
    result = add_calc.exec()  # Execute the addition calculation
...
def test_add_calculation_execute_negative(mock_addition):
    ...
    with pytest.raises(Exception) as exc_info:
        add_calc.exec()  # Attempt to execute addition, expecting an exception

    # Verify that the exception message is as expected
    assert str(exc_info.value) == "Addition error"


@patch.object(Operation, 'sub')
def test_subtract_calculation_execute_positive(mock_subtraction):
    ...
    subtract_calc = SubCalculation(a, b)

    # Act
    result = subtract_calc.exec()
...
def test_multiply_calculation_execute_positive(mock_multiplication):
    ...
    multiply_calc = MulCalculation(a, b)

    # Act
    result = multiply_calc.exec()
...
def test_divide_calculation_execute_positive(mock_division):
    ...
    divide_calc = DivCalculation(a, b)

    # Act
    result = divide_calc.exec()
...
def test_factory_creates_subtract_calculation():
    ...
    # Act
    calc = CalculationFactory.create_calculation('sub', a, b)

    # Assert
    assert isinstance(calc, SubCalculation)
    assert calc.a == a
    assert calc.b == b
...
def test_factory_creates_multiply_calculation():
    ...
    # Act
    calc = CalculationFactory.create_calculation('mul', a, b)

    # Assert
    assert isinstance(calc, MulCalculation)
    assert calc.a == a
    assert calc.b == b
...
def test_factory_creates_divide_calculation():
    ...
    # Act
    calc = CalculationFactory.create_calculation('div', a, b)

    # Assert
    assert isinstance(calc, DivCalculation)
    assert calc.a == a
    assert calc.b == b
...
@patch.object(Operation, 'add', return_value=15.0)
...
@patch.object(Operation, 'sub', return_value=5.0)
def test_calculation_str_representation_subtraction(mock_subtraction):
    ...
    expected_str = f"{subtract_calc.__class__.__name__}: {a} Sub {b} = 5.0"
    assert calc_str == expected_str


@patch.object(Operation, 'mul', return_value=50.0)
def test_calculation_str_representation_multiplication(mock_multiplication):
    ...
    expected_str = f"{multiply_calc.__class__.__name__}: {a} Mul {b} = 50.0"
    assert calc_str == expected_str


@patch.object(Operation, 'div', return_value=2.0)
def test_calculation_str_representation_division(mock_division):
    ...
    expected_str = f"{divide_calc.__class__.__name__}: {a} Div {b} = 2.0"
    assert calc_str == expected_str
...
def test_calculation_repr_representation_subtraction():
    ...
    expected_repr = f"{SubCalculation.__name__}(a={a}, b={b})"
...
def test_calculation_repr_representation_division():
    ...
    # Assert
    # The __repr__ should display the class name and the operands in a clear format
    expected_repr = f"{DivCalculation.__name__}(a={a}, b={b})"
    assert calc_repr == expected_repr
...
@pytest.mark.parametrize("calc_type, a, b, expected_str", [
    ...
    ('sub', 10.0, 5.0, "SubCalculation: 10.0 Sub 5.0 = 5.0"),
    ('mul', 10.0, 5.0, "MulCalculation: 10.0 Mul 5.0 = 50.0"),
    ('div', 10.0, 5.0, "DivCalculation: 10.0 Div 5.0 = 2.0"),
])
@patch.object(Operation, 'add', return_value=15.0)
@patch.object(Operation, 'sub', return_value=5.0)
@patch.object(Operation, 'mul', return_value=50.0)
